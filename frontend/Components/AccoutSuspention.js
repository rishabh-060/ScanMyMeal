...
const UpdateUserToAdmin = ({data, close, fetchAllUsers}) => {
  ...
  const handleSuspendUser = async () => {
    ...
    try {
        ...
        if (responseData.success) {
            AlertMessage('Successfull' ,'User suspended successfully')
            if(close) close()
            if(fetchAllUsers) fetchAllUsers()
        } else {
            AlertMessage('Failed' ,'Failed to suspend user')
        }
    } catch (error) {
  ...
  const handleRemoveSuspention = async () => {
    setLoading(true)
    try {
        const response = await Axios({
            ...summaryApi.activateUser,
            data: {
                userEmail: data.email
            }
        })
        ...
        const { data: responseData } = response;
        if (responseData.success) {
            AlertMessage('Successfull' ,'Suspension removed successfully')
            if(close) close()
            if(fetchAllUsers) fetchAllUsers()
        } else {
            AlertMessage('Failed' ,'Failed to remove suspension')
        }
    } catch (error) {
        AlertMessage('Failed' ,'Server error, please try again later')
    } finally {
  ...
  return (
    <section className='fixed top-0 bottom-0 left-0 right-0 bg-neutral-300/60 w-full h-full flex flex-col z-40 items-center justify-center'>
        <div className='bg-neutral-50 w-full lg:w-128 flex flex-col items-center p-5 pb-8 rounded-lg gap-6'>
            ...
            <p className={`text-sm font-semibold ${data.status !== 'Suspended' ? 'text-emerald-600' : 'text-neutral-700'} flex items-center gap-2`}>
                {data.status === 'Suspended' ? <FaUserShield /> : <FaUser />} {data.status}
            </p>
            </div>
            ...
            <div className='mt-6 flex flex-col items-center gap-3'>
            {
                data.status === 'Suspended' ? (
                    <p className='text-sm text-emerald-600 font-semibold'>{data.name}.</p>
                ) : (
                    <p className='text-sm text-neutral-700'>Click the button below to suspend the user.</p>
                )
            }
            <div className='w-full grid grid-col-2 gap-4'>
                <button
                    onClick={handleSuspendUser}
                    disabled={loading ||data.status === 'Suspended'}
                    className={`px-6 py-2.5 rounded-lg font-semibold text-white ${data.status === 'Suspended' ? 'bg-neutral-300 cursor-not-allowed' : 'bg-amber-500 hover:bg-amber-600'}  transition ${
                    loading ? 'opacity-60 cursor-not-allowed' : ''
                    }`}
                >
                    {loading ? 'Updating...' : 'Suspend User'}
                </button>
                
                <button
                    onClick={handleRemoveSuspention}
                    disabled={loading || data.status !== 'Suspended'}
                    className={`px-6 py-2.5 rounded-lg font-semibold text-white ${data.status !== 'Suspended' ? 'bg-neutral-300 cursor-not-allowed' : 'bg-amber-500 hover:bg-amber-600'}  transition ${
                    loading ? 'opacity-60 cursor-not-allowed' : ''
                    }`}
                >
                    {loading ? 'Updating...' : 'remove Suspension'}
                </button>
            </div>
            </div>
        </div>
