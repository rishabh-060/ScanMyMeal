...
const CardProduct = ({data}) => {
  ...
  return (
        <Link href={url} className="w-38 bg-white rounded-lg p-2.5 min-h-36 max-h-66 grid gap-1.5 hover:shadow-lg shrink-0 transition-transform duration-150 cursor-pointer">
            <div className="bg-white rounded h-32 relative overflow-hidden">
              ...
              {data.discount && parseFloat(data.discount) > 0 && (
                <span className="absolute top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full shadow-md animate-bounce transition-transform duration-300">
                  {data.discount}% OFF
                </span>
              )}
            </div>
            
            <div className="text-ellipsis line-clamp-2 text-sm font-medium text-neutral-800 leading-3.5 max-h-8 mt-1.5">{data?.name}</div>
